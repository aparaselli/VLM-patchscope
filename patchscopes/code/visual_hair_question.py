...
celeba_df = celeba_df.iloc[:10] #FOR TESTING [:1000]
...
prompt_target = "<image>\n{Black, Blond, Brown, Gray, Red, NONE} Out of the listed hair colors the one that best describes this person is"#"The Hair color of this person is x"#"{Black, Blond, Brown, Gray} Out of the listed hair colors the one that best describes this person is x"
position_source = -1
position_target = -1
prompt_source = "<image>\n{Black, Blond, Brown, Gray, Red, NONE} Out of the listed hair colors the one that best describes this person is"#<image>\n{Black, Blond, Brown, Gray, Red, NONE} Out of the listed hair colors the one that best describes this person is
red_head = Image.open(celeba_df['image_path'].iloc[1]).convert("RGB")
...
for layer_source in range(32): #only trying one for now
    ...
    celeba_df[f"inspect_layer{layer_source}"] = celeba_df.image_path.progress_apply(
        lambda img_path: inspect(mt, prompt_source, prompt_target, layer_source, layer_target,
                                      ...
                                      generation_mode=generation_mode, max_gen_len=max_gen_len, verbose=False,image_target=red_head,image_source=Image.open(img_path).convert("RGB")))

celeba_df.to_csv("small-CELEB_A-haircolor-processing-concrete-5_llava-7b-v1.1.csv", index=False)