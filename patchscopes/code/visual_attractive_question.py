...
celeba_df = celeba_df.iloc[:14] #FOR TESTING [:1000]
...
chat_source = [
    {
      "role": "user",
      "content": [
          {"type": "text", "text": "{Dark, Blonde, Brunette, Gray, Red, NONE} Out of the listed hair colors which one best describes the person in the image?"},
          {"type": "image"},
        ],
    },
]
...
for layer_source in range(1): #only trying one for now
    ...
    celeba_df[f"inspect_layer{layer_source}"] = celeba_df.image_path.progress_apply(
        lambda img_path: inspect(mt, prompt_source, prompt_target, layer_source, layer_target,
                                      ...
                                      generation_mode=generation_mode, max_gen_len=max_gen_len, verbose=False,image_source=Image.open(img_path).convert("RGB"),image_target=Image.open(img_path).convert("RGB")))

celeba_df.to_csv("repeat-mess-nonsense-question-rep.csv", index=False)