...
#choose image for source
img_source = celeba_df['image_path'].iloc[8]
img_source = Image.open(img_source).convert("RGB")

#choose picture for target
...
for layer_source in range(1): #only trying one for now
    ...
    celeba_df[f"inspect_layer{layer_source}"] = celeba_df.image_path.progress_apply(
        lambda img_path: inspect(mt, prompt_source, prompt_target, layer_source, layer_target,
                                      ...
                                      generation_mode=generation_mode, max_gen_len=max_gen_len, verbose=False,image_source=img_source,image_target=Image.open(img_path).convert("RGB")))

celeba_df.to_csv("selfie-surg-hair.csv", index=False)
